...
@section('header-title', 'Detail Kasus Penyakit')
...
@section('content')
    <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 32px; max-width: 1100px; margin: 0 auto;">
        ...
        <div class="card" style="display: flex; flex-direction: column; gap: 24px;">
            <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--card-border); padding-bottom: 16px;">
                <div style="display: flex; align-items: center; gap: 12px;">
                    <a href="{{ route('kesehatan.reports.index') }}" class="btn btn-secondary btn-sm" style="padding: 8px 12px;"><i class="ri-arrow-left-line"></i> Kembali</a>
            ...
            <div style="border-top: 1px solid var(--card-border); padding-top: 20px;">
                <h4 style="font-size: 14px; font-weight: 700; color: var(--text-secondary); text-transform: uppercase; margin-bottom: 12px;">Rekomendasi Medis (Dokter)</h4>
                @if($report->treatment_recommendation)
                    <p style="font-size: 14px; line-height: 1.6; background-color: rgba(16, 185, 129, 0.05); color: #065f46; padding: 16px; border-radius: 12px; border: 1px solid rgba(16, 185, 129, 0.15);">
                        <i class="ri-shield-check-fill" style="font-size: 18px; vertical-align: middle;"></i>
        ...
        <div class="card" style="height: fit-content; display: flex; flex-direction: column; gap: 20px;">
            ...
            <div>
                <span style="font-size: 12px; color: var(--text-secondary)">Dilaporkan Oleh:</span>
                <p style="font-size: 14px; font-weight: 600;">{{ $report->reporter->name }} ({{ ucfirst($report->reporter->role) }})</p>
                <small style="color: var(--text-secondary)">Pada {{ $report->created_at->format('d/m/Y H:i') }} WIB</small>
            </div>
            ...
            @if($report->latitude && $report->longitude)
                <div style="border-top: 1px solid var(--card-border); padding-top: 16px;">
                    <span style="font-size: 12px; color: var(--text-secondary)"><i class="ri-map-pin-line"></i> Koordinat GIS Pasien:</span>
                    <p style="font-size: 13px; font-family: monospace; margin-top: 4px;">Lat: {{ $report->latitude }}, Lng: {{ $report->longitude }}</p>
                    @if($report->village)
