    ...
    <table>
        ...
        <tbody>
            @foreach($data as $index => $rec)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td><strong>{{ $rec->child->name }}</strong><br><small>Gender: {{ $rec->child->gender }}</small></td>
                    <td>{{ $rec->child->date_of_birth->format('d/m/Y') }}<br><small>{{ $rec->child->getAgeMonths() }} Bln</small></td>
                    <td>{{ $rec->child->parent->name }}</td>
                    ...
                    <td><strong>{{ ucfirst($rec->status) }}</strong></td>
                </tr>
            @endforeach
        </tbody>
    </table>
