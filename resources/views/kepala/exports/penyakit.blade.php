    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Penyakit</th>
                <th>Tingkat</th>
                <th>Rekomendasi Tindakan Medis</th>
            </tr>
        </thead>
        <tbody>
            @foreach($data as $index => $rep)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $rep->report_date->format('d/m/Y') }}</td>
                    <td><strong>{{ $rep->patient_name }}</strong><br><small>NIK: {{ $rep->patient_nik }}</small></td>
                    <td>{{ $rep->patient_age }} Thn</td>
                    <td>{{ $rep->patient_gender }}</td>
                    <td>{{ $rep->patient_address }}</td>
                    <td><strong>{{ $rep->diseaseType->name }}</strong></td>
                    <td>{{ ucfirst($rep->severity) }}</td>
                    <td>{{ $rep->treatment_recommendation ?? 'Menunggu rekomendasi medis' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
